...
void main() {
  test('largeFileHunterProvider filters by threshold and sorts descending', () {
    ...
    expect(
      largeFiles.map((file) => file.filename),
      ['huge.zip', 'medium.zip'],
    );
  });
  ...
  test('duplicateGroupsProvider returns empty before any scan', () async {
    ...
    await expectLater(container.read(duplicateGroupsProvider.future), isEmpty);
  });

  test('duplicateGroupsProvider detects duplicates from scanned files', () async {
  ...
  test('recommendationsProvider is empty until a scan exists', () async {
    ...
    await expectLater(container.read(recommendationsProvider.future), isEmpty);
  });

  test('recommendationsProvider builds duplicate and APK recommendations', () async {
